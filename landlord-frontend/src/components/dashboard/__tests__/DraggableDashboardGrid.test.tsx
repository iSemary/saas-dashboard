...
describe("DraggableDashboardGrid", () => {
  ...
  it("debounces API save on layout change", async () => {
    renderGrid();
    // Wait for mount with real timers before switching to fake
    await screen.findByTestId("trigger-layout-change");
    ...
    try {
      ...
      // API should NOT have been called yet (debounce pending)
      expect(vi.mocked(api.post)).not.toHaveBeenCalled();

      act(() => { vi.advanceTimersByTime(700); });

      // After debounce, exactly one POST
  ...
  it("dispatches resize event on layout change", async () => {
    ...
    renderGrid();
    const trigger = await screen.findByTestId("trigger-layout-change");
  ...
  it("ignores corrupted localStorage JSON", async () => {
    ...
    // findByText uses real timers + polls — no waitFor needed
