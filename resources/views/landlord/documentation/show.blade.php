...
@section('title', 'Documentation - ' . $file)
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('landlord.documentation.index') }}">@translate('documentation')</a></li>
                        <li class="breadcrumb-item active">{{ basename($file, '.md') }}</li>
                    </ol>
                </div>
            </div>
        </div>
                                ...
                                @if(count($files) > 0)
                                    @php
                                        ...
                                        foreach($files as $fileItem) {
                                            if(empty($fileItem['directory'])) {
                                                $groupedFiles['root'][] = $fileItem;
                                            } else {
                                                $groupedFiles[$fileItem['directory']][] = $fileItem;
                                            }
                                        }
                                    @endphp
                                        ...
                                        @if($directory !== 'root')
                                            <div class="nav-group">
                                                ...
                                                <ul class="nav-group-items">
                                                    @foreach($directoryFiles as $fileItem)
                                                        <li class="nav-item">
                                                            <a href="{{ route('landlord.documentation.show', $fileItem['path']) }}" 
                                                               class="nav-link {{ $fileItem['path'] === $file ? 'active' : '' }}">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('landlord.documentation.index') }}" class="btn btn-tool">
                                    <i class="fas fa-arrow-left"></i>
                                    @translate('back_to_documentation')
                                </a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div id="documentation-content">
                                <div class="markdown-content">
                                    {!! (new \App\Http\Controllers\Landlord\DocumentationController())->parseMarkdown($content) !!}
                                </div>
                            </div>
                        </div>
                    </div>
