...
const TypingTextContext = React.createContext<TypingTextContextType | null>(null);

function useTypingText() {
  const context = React.useContext(TypingTextContext);
  if (!context) {
    return { isTyping: false, setIsTyping: () => {} };
  }
  return context;
}
...
function TypingText({
  ...
  return (
    <TypingTextContext.Provider value={{ isTyping, setIsTyping }}>
