...
const STORAGE_KEY = "dashboard_layout_sms_marketing";

function buildDefaultLayouts(): ResponsiveLayouts {
  const keys = ["campaigns", "contacts", "templates", "draft", "sent", "delivery_rate", "failure_rate", "total_cost", "opted_out", "lists", "scheduled"];
...
export default function SmsMarketingDashboardPage() {
  ...
  const cards = [
    { key: "campaigns", label: "Total Campaigns", value: stats.total_campaigns, icon: Megaphone, color: "text-red-600" },
    ...
    { key: "sent", label: "Sent Campaigns", value: stats.sent_campaigns, icon: Smartphone, color: "text-green-600" },
    { key: "delivery_rate", label: "Delivery Rate", value: `${stats.delivery_rate}%`, icon: TrendingUp, color: "text-emerald-600" },
    { key: "failure_rate", label: "Failure Rate", value: `${stats.failure_rate}%`, icon: TrendingUp, color: "text-red-600" },
    { key: "total_cost", label: "Total Cost", value: `$${stats.total_cost?.toLocaleString() ?? '0'}`, icon: DollarSign, color: "text-amber-600" },
    { key: "opted_out", label: "Opted Out", value: stats.opted_out_contacts, icon: Smartphone, color: "text-orange-600" },
    { key: "lists", label: "Contact Lists", value: stats.total_contact_lists, icon: Users, color: "text-cyan-600" },
    { key: "scheduled", label: "Scheduled", value: stats.scheduled_campaigns, icon: Smartphone, color: "text-indigo-600" },
  ];
