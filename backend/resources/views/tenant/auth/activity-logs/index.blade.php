...
@section('title', $title ?? translate('activity_logs'))
    ...
    <div class="card-header">
        <div class="row">
            ...
            <div class="col-8 text-right">
                <a href="{{ $isAllActivities ? '#' : route('tenant.activity-logs.index', $id) . '?page=1' }}"
                    class="{{ $isAllActivities ? 'text-muted' : 'text-primary' }}">
                    <i class="fas fa-list-ul"></i>
                    @translate('all_activities')
                </a>
                <span class="mx-2 text-muted">|</span>
                <a href="{{ $isDeletedActivities ? '#' : route('tenant.activity-logs.index', $id) . '?type=deleted&page=1' }}"
